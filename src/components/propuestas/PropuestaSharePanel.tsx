...
type PropuestaSharePanelProps = {
  slug: string;
  operatorEmail?: string;
  titulo: string;
};

export function PropuestaSharePanel({
  slug,
  operatorEmail,
  titulo,
}: PropuestaSharePanelProps) {
  ...
  const shareUrl = share
    ? `${typeof window !== "undefined" ? window.location.origin : ""}/propuestas/v/${share.token}`
    : "";
  ...
  const loadShare = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/propuestas/share?${params}`);
  ...
  const postAction = async (
    ...
    setError("");
    try {
      const response = await fetch("/api/propuestas/share", {
        method: "POST",
  ...
  return (
    <>
      <button
        ...
        className="gabi-no-print fixed bottom-6 right-6 z-40 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-[12px] font-semibold text-slate-700 shadow-lg transition hover:shadow-xl"
      >
      ...
      {open ? (
        <div className="gabi-no-print fixed inset-0 z-50 flex items-end justify-center bg-black/30 p-4 sm:items-center">
          <div
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
              <div>
                ...
                <h2 id="share-panel-title" className="text-lg font-bold text-slate-800">
                  Compartir propuesta
                </h2>
            ...
            <div className="space-y-4 px-5 py-4">
              <p className="text-sm leading-relaxed text-slate-600">
                Enlace privado + código de 6 dígitos. El desarrollador necesita ambos para entrar.
              </p>
                  ...
                  {codigo ? (
                    <div className="rounded-xl border border-[#6cc24a]/30 bg-[#6cc24a]/5 p-3">
                      <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">
                        Código de acceso (guárdalo ahora)
                      </p>
                      ...
                      <p className="mt-2 text-[11px] text-slate-500">
                        El código no se vuelve a mostrar. Regenera uno nuevo si lo perdiste.
                      </p>
                    </div>
                  ) : (
                    <p className="text-[12px] text-slate-500">
                      Enlace activo. Regenera el código si necesitas compartirlo de nuevo.
                    </p>
                  )}
