...
function orderInvesttiDesarrollos(items: DesarrolloRecord[]): DesarrolloRecord[] {
  ...
  return [...items].sort(
    (a, b) => (order.get(a.id as (typeof INVESTTI_CATALOG_DESARROLLO_IDS)[number]) ?? 99) -
      (order.get(b.id as (typeof INVESTTI_CATALOG_DESARROLLO_IDS)[number]) ?? 99),
  );
}
    ...
    <main className="min-h-screen bg-[#F8FAFC]" style={{ color: primary }}>
      <header className="border-b border-black/8 bg-white px-5 py-3 shadow-sm md:px-8">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <Image
              src={portal?.logo ?? INVESTTI_GRUPO_LOGO}
              alt="Grupo Investti"
              width={420}
              height={260}
              priority
      ...
      <section className="mx-auto max-w-5xl px-5 py-5 md:px-8 md:py-6">
        <div className="mb-4 flex items-center gap-2 text-sm text-slate-500">
          <Calculator className="h-4 w-4 shrink-0" style={{ color: accent }} />
          {loadingCatalog
            ? "Cargando catálogo…"
            : `${desarrollosDisponibles.length} desarrollo${desarrollosDisponibles.length === 1 ? "" : "s"} disponible${desarrollosDisponibles.length === 1 ? "" : "s"} para cotizar`}
        </div>
        ...
        <div className="grid gap-2.5 sm:grid-cols-2 sm:gap-3">
          {loadingCatalog ? (
            ...
            <div className="rounded-xl border border-dashed border-black/15 bg-white p-6 text-center sm:col-span-2">
              <p className="font-bold">Sin desarrollos disponibles</p>
              <p className="mt-1.5 text-sm leading-relaxed text-slate-500">
                No se encontraron desarrollos Investti para tu acceso. Verifica el PIN o contacta
                a BBR Habitarea.
              </p>
            </div>
          ) : (
